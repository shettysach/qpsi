...
`fp8` means TorchAO dynamic W8A8 E4M3 on eligible `nn.Linear` layers, not every tensor in the component. Linear dimensions must be multiples of 16 and at least 64. The VLM's tied output/embedding weight is excluded. Norms, embeddings, nonlinear operations, and unsupported projections retain their released precision. The action output linear layer is included when eligible. FP32 biases on selected FP8 linears are cast to BF16 because the FP8 matrix multiplication kernel requires a Half or BF16 bias. Each run's `summary.json` records the converted layer names, parameter count, and bias casts.
